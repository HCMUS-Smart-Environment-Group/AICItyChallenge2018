...
def get_velocity_one_object(obj, collision, lines, linenames, lineidx, dist, find):
    ...
    for i in xrange(1, N):
        ...
        s = s.replace(" ", "")
        v[i - 1] = ALPHA * dist[s] * 30 * 0.681818 / max((last - first), 1)
        mean.append(v[i - 1])
...
def smooth(info, velocity, side):
    for i in xrange(velocity.shape[0]):
        if velocity[i] > 50:
            velocity[i] = 50
    ...
    for i, obj in enumerate(info):
        if velocity[i] == np.inf:
            if side[i] == 2:
                velocity[i] = 0 # velocity[i])
            if side[i] == 0 or side[i] == 1:
                velocity[i] = mean # velocity[i])
            if side[i] == -1:
                velocity[i] = mean
        if velocity[i] >= 50:
            velocity[i] = mean
...
if __name__ == '__main__':
    ...
    video_names = [video_name for video_name in video_names if video_name[:4] == 'Loc4']
    video_names.sort()
